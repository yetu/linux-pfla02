...
static void update_display_setting(void)
{
	...
	for (i = 0; i < num_registered_fb; i++) {
		fbi = registered_fb[i];
		pr_err("%s(%d) YETU  allocate video device %d\n", __func__, __LINE__, i);
...
static int mxc_vout_setup_output(struct mxc_vout_dev *dev)
{
	...
	for (i = 0; i < num_registered_fb; i++) {
		...
		if (!vout->vfd) {
			pr_err("%s(%d) YETU cannot allocate video device %d\n", __func__, __LINE__, i);
			ret = -ENOMEM;
			break;
		}
		...
		*vout->vfd = mxc_vout_template;
		...
		if (video_register_device(vout->vfd,
			...
			ret = -ENODEV;
			pr_err("%s(%d) YETU cannot register device video%d\n", __func__, __LINE__, video_nr+i);
			dump_stack();
			break;
		}
